    ...
    <div class="base_form">
        ...
        <div class="base_query_oneLine">
            ...
            <div class="query_right">
                <span>评价学生:</span>
                <select class="commInput" v-model="student1Id">
        ...
        <div class="dialog-div">
            <table class="dialog_content">
                <tr>
                    ...
                    <td colspan="1">
                        <select class="commInput" v-model="editedItem.student1Id">
                            <option value="0">请选择...</option>
                            <option v-for="item in studentList" :key="item.id" :value="item.id">
                                {{ item.title }}
                            </option>
...
import { type OptionItem, type AssessmentItem } from "~/models/general";
import {
    assessmentDelete,
    getAssessmentList,
    assessmentSave,
...
export default defineComponent({
    ...
    computed: {
        filteredStudentList() {
            return this.studentList.filter(item => item.id !== this.student1Id);
        },
        filteredEditStudentList() {
            return this.studentList.filter(item => item.id !== this.editedItem.student1Id);
        }
    },
    created() {
        this.initialize();
