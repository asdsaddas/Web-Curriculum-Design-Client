...
//选课信息删除后台数据请求方法
export async function fitDelete(fitId: number): Promise<DataResponse> {
  const res = await generalRequest("/api/fit/fitDelete", {
    fitId: fitId,
  });
  return res as DataResponse;
}
//选课记录保存后台数据请求方法
export async function fitSave(
  fitId: number | null,
  studentId: number,
  courseId: number,
): Promise<DataResponse> {
    const res = await generalRequest("/api/fit/fitSave", {
    fitId: fitId,
    studentId: studentId,
    courseId: courseId,
...
//获取选课信息列表后台数据请求方法
export async function getFitList(
  studentId: number | null,
  courseId:number|null,
): Promise<[]> {
  const res = await generalRequest("/api/fit/getFitList", {
    studentId: studentId,
    courseId:courseId,
  });
  return res.data as [];
